...
def setup_platform(hass, config, add_entities, discovery_info=None):
    ...
    epson = EpsonEntity(config.get(CONF_NAME), config.get(CONF_PORT))
    add_entities([epson])
    return True
...
class EpsonEntity(MediaPlayerEntity):
    ...
    def __init__(self, name, port):
        ...
        self._ser_io = io.TextIOWrapper(
            io.BufferedRWPair(ser, ser), line_buffering=True, newline="\r"
        )
    ...
    def select_source(self, source):
        ...
        self.serial_command(
            ...
            + list(self._source_list.keys())[
                list(self._source_list.values()).index(source)
            ]
        )
